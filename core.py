...
class CherryTree:
    ...
    def nodes_add_from_html_file(self, action):
        ...
        selection_data = re.sub(cons.BAD_CHARS, "", html_string)
    ...
    def nodes_add_from_html_toc(self, action):
        """Add Nodes Parsing a TuxCards File"""
        filepath = support.dialog_file_select(curr_folder=self.file_dir, parent=self.window)
        if filepath is None: return
        html_toc = imports.HtmlTocHandler(filepath)
        cherrytree_string = html_toc.get_cherrytree_xml()
        # Xml2Node(self).from_xml_string_to_buffer(cherrytree_string)
        self.nodes_add_from_cherrytree_data(cherrytree_string)
