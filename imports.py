...
class HtmlTocHandler():
    ...
    def parse_toc(self):
        ...
        page = codecs.open(self.filepath, encoding='utf-8')
        html = page.read()

        cleaner = Cleaner(remove_tags=['b', 'link', 'script', 'img'], kill_tags=['img'])
        html = html.replace('\00', '')

        clean_html = cleaner.clean_html(html)
        tree = lxml.html.etree.HTML(clean_html)

        xpath = '//div[@id="tree"]/div[@class="item"]'
        xpath_details = './a[@target="fdata"]'
        xpath_subitems = './div[@class="subitems"]/div[@class="item"]'

        main = tree.xpath(xpath)[0]
        main_title = main.xpath(xpath_details)[0]

        def get_items(element, xpath):
            result = []
            for tag in element.xpath(xpath):
                tag_details = tag.xpath(xpath_details)[0]
                r = dict()
                r['title'] = tag_details.text
                r['href'] = self.get_url(tag_details)
                r['sub'] = get_items(tag, xpath)
                result.append(r)
            return result

        content = dict()
        content['href'] = self.get_url(main_title)
        content['title'] = main_title.text
        content['sub'] = get_items(main, xpath_subitems)

        self.toc = content

    def add_folder(self, subs):
        """Add nodes from plain text files in a Folder"""
        for sub in subs:
            node_name = sub['title']
            node_content = self.get_node_content(sub)
            self.add_node_with_content(node_name, node_content)
            if sub.get('sub'):
                self.add_folder(sub.get('sub'))
            self.nodes_list.pop()

    def add_node_with_content(self, name, content):
        """Append Node and Fill Content"""
        self.nodes_list.append(self.dom.createElement("node"))
        self.nodes_list[-1].setAttribute("name", name)
        self.nodes_list[-1].setAttribute("prog_lang", cons.CUSTOM_COLORS_ID)
        self.nodes_list[-2].appendChild(self.nodes_list[-1])

        dom_iter = self.dom.createElement("rich_text")
        self.nodes_list[-1].appendChild(dom_iter)
        text_iter = self.dom.createTextNode(content)
        dom_iter.appendChild(text_iter)

    def get_node_content(self, tag):
        link = tag['href']

        data = ''
        with open(link, 'r') as f:
            selection_data = f.read()
            html_import = HTMLFromClipboardHandler(self)
            data = html_import.get_clipboard_selection_xml(selection_data)
        return data
